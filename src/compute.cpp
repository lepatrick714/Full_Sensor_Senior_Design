...
void calculate_cp(int tempCnt, double &CPMethane, double &CPCO2, double &CPEthane, double &thermConMethane, double &thermConEthane, double &thermConCO2) {
            if (tempCnt == -20) {
            ...
            for(int i = -20; i <= 80; i = i + 20) {
                ...
                for(int pressure = 500; pressure <= 3000; pressure = pressure + 500) {
                    ...
                    for(int j = 0; j < v.size(); ++j) {
                        ...
                        MethaneRatio = texasPipleineMethane*percentTexas + rockyPipelineMethane*percentRocky + peruvianLNGMethane*percentPeruvian + highAssociated1Methane*percentHigh1 + highAssociated2Methane*percentHigh2;

//Here the percent variable are the percentages the permutations should be generating

                          EthaneRatio = texasPipleineEthane*percentTexas + rockyPipelineEthane*percentRocky + peruvianLNGEthane*percentPeruvian + highAssociated1Ethane*percentHigh1 + highAssociated2Ethane*percentHigh2;



                        CO2Ratio = texasPipleineCO2*percentTexas + rockyPipelineCO2*percentRocky + peruvianLNGCO2*percentPeruvian + highAssociated1CO2*percentHigh1 + highAssociated2CO2percentHigh2;
                        ...
                        file << "The ratio is " << MethaneRatio << "% Methane and " << CO2Ratio  << "% CO2 and " << EthaneRatio  << "% Ethane." << endl;
                        
                        CpMixture = (CPMethane * MethaneRatio) + (CPCO2 * CO2Ratio) + (CPEthane*EthaneRatio); // important we want this
                        ...
                        avgmolecularweight = MethaneRatio * molarmassmethane + CO2Ratio * molarmassCO2 + EthaneRatio * molarmassethane;
                        ...
                        z=1;
                                               mixtureDensity = ((avgmolecularweight * pressure)/ (z*gasDensityvalue* temp)) * sidensityconvert; // important we want this
                        file << "The molar density is : " << mixtureDensity << endl;
                        ThermalConducitivyMixture = ((thermConMethane * MethaneRatio) + (thermConEthane * EthaneRatio) + ( thermConCO2 * CO2Ratio))/1000; // important we want this
                        ...
                        file1 << i << ", " << pressure << ", " << count << ", " << CombinationFossil << ", " << CombinationAnaerobic << ", " << CombinationLandfill;
                        file1 << ", , " << MethaneRatio * 100 << ", " << CO2Ratio * 100 << ", " << EthaneRatio * 100<< ", , " << ThermalConducitivyMixture << ", " << CpMixture << ", " << CvMixture << ", " << mixtureDensity << ", " << specificgravity << ", " << soundVelocity << endl; 
